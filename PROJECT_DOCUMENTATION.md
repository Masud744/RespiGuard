...
5. [Machine Learning Pipeline & Model Architectures / মেশিন লার্নিং পাইপলাইন](#5-machine-learning-pipeline--model-architectures)
   ...
   - 5.3 Leak-Free Empirical Evaluation Benchmarks & Pre-Tuning Baseline Audit
   - 5.4 Quarantine of Historical Production Artifact & Non-Equivalence Proof
   - 5.5 Phase 5 Controlled Optimization Results (EXP-TUNE-01 to EXP-TUNE-04)
   - 5.6 Cross-Subject Phenotypic Trigger Divergence & Clinical Limitations
   - 5.7 Cryptographic Reproducibility Manifest & Release Tag
...
### 5.1 Proposed 2-Stage Hierarchical Classification Architecture
একটি সাধারণ ৩-ক্লাস মাল্টিক্লাস মডেলের বদলে RespiGuard একটি বিশেষায়িত **২-ধাপের হায়ারার্কিক্যাল আর্কিটেকচার (Two-Stage Hierarchical Pipeline)** ব্যবহার করে:
1. **ক্লিনিক্যাল গুরুত্বের অগ্রাধিকার:** চিকিৎসাবিজ্ঞানে একজন সুস্থ রোগীকে সতর্ক করা ক্ষতিকর নয়, কিন্তু একজন জরুরি মৃত্যুঝুঁকিতে থাকা Red রোগীকে ভুলবশত "Safe (Green)" বলে অবহেলা করা প্রাণঘাতী।
2. **ডিকাপল্ড অপ্টিমাইজেশন (Decoupled Optimization):** 
   - **Stage 1 (CatBoost Safety Gate):** ফলস নেগেটিভ শূন্যের কাছাকাছি নামিয়ে আনার জন্য At-Risk ক্লাসে কস্ট-সেন্সিটিভ পেনাল্টি ওয়েট দিয়ে অপ্টিমাইজ করা, যেন কোনো অসুস্থ রোগী ভুলবশত "Safe" বলে অবহেলা না পায়।
   - **Stage 2 (XGBoost Severity Triage):** মাঝারি ঝুঁকি (Yellow) এবং জরুরি বিপদ (Red)-এর সূক্ষ্ম শারীরিক পার্থক্যের ওপর বিশেষভাবে ফোকাস করে রোগীর জরুরি ইনহেলার অ্যালার্ট ট্রিগার করে।
...
যেখানে $w \ge 1.0$ হলো At-Risk ক্লাসের পেনাল্টি ওজন।
...
প্রাক-টিউনিং অডিটে প্রমাণিত হয়েছে যে ঐতিহাসিক ৮৪–৮৭% বা ৮৩.৫৬% ফলাফলগুলো পেশেন্ট-ওভারল্যাপ (Patient Overlap) এবং ডেমোগ্রাফিক শর্টকাটের কারণে অতি-উচ্চ ছিল। কঠোর সায়েন্টিফিক মানদণ্ডে অডিট সম্পন্ন করে তিনটি স্বতন্ত্র মূল্যায়ন ট্র্যাক (Evaluation Tracks) নির্ধারণ করা হয়েছে:

#### ট্র্যাক ১: কোয়ারেন্টাইনড প্রোডাকশন আর্টিফ্যাক্ট (`EXP-BASE-HIST-01` - Quarantined)
- **মডেল:** সংরক্ষিত প্রোডাকশন ফাইল `models/two_stage_asthma_model.joblib` (৯টি ফিচার)।
- **স্প্লিট:** রো-র্যান্ডম স্প্লিট (প্রশিক্ষণ ও টেস্ট সেটে একই রোগীর ডাটা মিশ্রিত ছিল)।
- **স্ট্যাটাস:** কোয়ারেন্টাইনড (নতুন রোগীর ক্ষেত্রে সাধারণীকরণের জন্য এটি অবৈধ)।
- **ফলাফল:** টেস্ট অ্যাকুরেসি ৮৩.৫৬%, ম্যাক্রো F1 ০.৭৭৯৪, রেড রিকল ৫০.০% (২/৪)।

#### ট্র্যাক ২: লিক-মুক্ত সম্পূর্ণ নতুন রোগীর হোল্ড-আউট (`EXP-BASE-PATIENT-02` - Frozen Baseline)
- **মডেল:** ৪টি ফিজিক্যাল সেন্সর (`temperature, humidity, pm2_5, pm10`) দ্বারা স্ক্র্যাচ থেকে প্রশিক্ষিত মডেল।
- **স্প্লিট:** সম্পূর্ণ নতুন ৪ জন রোগী টেস্টে (`[113, 343, 701, 867]`, $N = 219$), প্রশিক্ষণে ৯ জন রোগী ($N = 1,138$), শূন্য ওভারল্যাপ।
- **ফলাফল সারণী:**

| মডেল আর্কিটেকচার | টেস্ট অ্যাকুরেসি | ব্যালেন্সড অ্যাকুরেসি | ম্যাক্রো F1 | ওয়েটেড F1 | রেড রিকল (TP/Total) | রেড $\to$ গ্রিন ফলস নেগেটিভ | প্রেডিকশন অনুপাত (G / Y / R) |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **Majority-Class Baseline** | 49.32% | 33.33% | 0.2202 | 0.3258 | 0.0% (0/4) | 4/4 (100.0%) | 219 / 0 / 0 |
| **Multinomial Logistic Regression** | 48.40% | 32.79% | 0.3189 | 0.4702 | 0.0% (0/4) | 0/4 (0.0%) | 151 / 44 / 24 |
| **Random Forest Classifier** | 47.95% | 32.46% | 0.2862 | 0.4223 | 0.0% (0/4) | 0/4 (0.0%) | 174 / 42 / 3 |
| **Single-Stage XGBoost** | 49.32% | 33.37% | 0.2772 | 0.4092 | 0.0% (0/4) | 2/4 (50.0%) | 190 / 27 / 2 |
| **Proposed 2-Stage (`CatBoost + XGBoost`)** | **49.77%** | **33.69%** | **0.2844** | **0.4197** | **0.0% (0/4)** | **1/4 (25.0%)** | 187 / 32 / 0 |

#### ট্র্যাক ৩: অফিসিয়াল সেফগার্ডেড ক্রোনোলজিক্যাল বেঞ্চমার্ক (`EXP-BASE-CHRONO-03-SAFEGUARD`, $N = 290$)
- **মডেল:** ৪টি ফিজিক্যাল সেন্সর (`temperature, humidity, pm2_5, pm10`), প্রি-প্রসেসিং কেবল অতীত ট্রেইনিং ডাটাতে ফিট।
- **স্প্লিট:** প্রতিটি রোগীর প্রথম ৭৫% ক্যালেন্ডার দিন প্রশিক্ষণে ($N = 1,066$), শেষ ২৫% দিন টেস্টে ($N = 290$)। একক তারিখের রোগী ২১৭-কে সেফগার্ড নীতির আওতায় বাদ দেওয়া হয়েছে যাতে শতভাগ রোগীর নন-এম্পটি অতীত ও ভবিষ্যৎ ডাটা থাকে।
- **ফলাফল সারণী:**

| মডেল আর্কিটেকচার | টেস্ট অ্যাকুরেসি | ব্যালেন্সড অ্যাকুরেসি | ম্যাক্রো F1 | ওয়েটেড F1 | রেড রিকল (TP/Total) | রেড $\to$ গ্রিন ফলস নেগেটিভ | প্রেডিকশন অনুপাত (G / Y / R) |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **Majority-Class Baseline** | 57.59% | 33.33% | 0.2436 | 0.4209 | 0.0% (0/2) | 2/2 (100.0%) | 290 / 0 / 0 |
| **Multinomial Logistic Regression** | 41.03% | 46.52% | 0.2977 | 0.4089 | 50.0% (1/2) | 0/2 (0.0%) | 45 / 196 / 49 |
| **Random Forest Classifier** | 53.45% | 38.68% | 0.3527 | 0.5131 | 0.0% (0/2) | 0/2 (0.0%) | 68 / 215 / 7 |
| **Single-Stage XGBoost** | 57.24% | 40.95% | 0.3774 | 0.5530 | 0.0% (0/2) | 0/2 (0.0%) | 81 / 209 / 0 |
| **Proposed 2-Stage (`CatBoost + XGBoost`)** | **55.86%** | **39.92%** | **0.3692** | **0.5412** | **0.0% (0/2)** | **0/2 (0.0%)** | 82 / 207 / 1 |

---

### 5.4 Quarantine of Historical Production Artifact & Non-Equivalence Proof

সংরক্ষিত প্রোডাকশন মডেল (`models/two_stage_asthma_model.joblib`) এবং নতুন ৪-সেন্সর মডেলের মধ্যে সমতা (Equivalence) সরাসরি তুলনা করে খণ্ডন করা হয়েছে:
- **ইনপুট ডাইমেনশন:** প্রোডাকশন মডেলে ৯টি ফিচার ব্যবহৃত হয়েছিল (যাতে ছিল ডেমোগ্রাফিক শর্টকাট ও সার্কুলার পিক ফ্লো বেসলাইন), অথচ নতুন ফ্রেশ অডিট পাইপলাইনে কঠোরভাবে ৪টি বাস্তব ফিজিক্যাল সেন্সর ব্যবহৃত।
- **প্রেডিকশন অমিল:** অভিন্ন ২১৯টি টেস্ট স্যাম্পলে দুই মডেলের প্রেডিকশন তুলনা করলে দেখা যায় **৮৯টি স্যাম্পলে (৪০.৬৪%) প্রেডিকশন সম্পূর্ণ ভিন্ন**।
- **সিদ্ধান্ত:** প্রোডাকশন মডেল এবং নতুন ফ্রেশ অডিট মডেল কোনোভাবেই সমতুল্য নয়; সংরক্ষিত মডেলটির ৮৩.৫৬% ফলাফলকে আনুষ্ঠানিকভাবে কোয়ারেন্টাইন করা হয়েছে।

---

### 5.5 Phase 5 Controlled Optimization Results (`EXP-TUNE-01` to `EXP-TUNE-04`)

প্রাক-টিউনিং বেসলাইন অডিট অনুমোদনের পর ৪টি নিয়ন্ত্রিত অপ্টিমাইজেশন এক্সপেরিমেন্ট পরিচালনা করা হয়:

| এক্সপেরিমেন্ট আইডি | মডেল কনফিগারেশন | ব্যবহৃত ফিচার | পেশেন্ট হোল্ড-আউট অ্যাকুরেসি ($N=219$) | ক্রোনোলজিক্যাল অ্যাকুরেসি ($N=290$) | রেড $\to$ গ্রিন ফলস নেগেটিভ | প্রধান অগ্রগতি ও ফলাফল |
| :--- | :--- | :---: | :---: | :---: | :---: | :--- |
| **`EXP-TUNE-01A`** | Two-Stage (Balanced Cat + Weighted XGB) | 4 Sensors | 50.68% | 53.45% | **0/4 (0.0%)** | কস্ট-সেন্সিটিভ ওজনে প্রাণঘাতী ফলস নেগেটিভ ২৫% থেকে ০%-এ নেমে আসে। |
| **`EXP-TUNE-01B`** | Two-Stage (Dual Balanced CatBoost) | 4 Sensors | **51.60%** | 53.79% | **0/4 (0.0%)** | পেশেন্ট হোল্ড-আউটে সর্বোচ্চ ব্যালেন্সড অ্যাকুরেসি (৩৪.৯৩%)। |
...
অডিটে একটি গভীর ক্লিনিক্যাল সত্য উন্মোচিত হয়েছে যা ৪টি পরিবেশগত সেন্সরের সাধারণীকরণ সীমাবদ্ধতা নির্দেশ করে:

1. **ট্রিগার বৈচিত্র্য (Endotype Divergence):**
   - **ট্রেইনিং কোহর্ট (রোগী ১৯০):** এই রোগীর অ্যাজমা অ্যাটাক বা রেড জোন তৈরি হয়েছিল **উষ্ণ আবহাওয়া ($১২.৪^\circ\text{C}$)** এবং **তীব্র বায়ুদূষণের স্পাইকে ($PM_{2.5}$ গড় ৮.৮, সর্বোচ্চ ২৫.৪ $\mu g/m^3$)**। মডেল শিখেছে যে দূষণ বাড়লে রেড ঝুঁকি বাড়ে।
   - **টেস্ট কোহর্ট (রোগী ৩৪৩):** এই রোগীর ৪টি রেড জোন তৈরি হয়েছিল **তীব্র শীত ও শৈত্যপ্রবাহে ($১.৭৫^\circ\text{C} - ৯.৫^\circ\text{C}$, গড় ৫.০$^\circ\text{C}$)** কিন্তু সেই সময় **বাতাস সম্পূর্ণ পরিষ্কার ছিল ($PM_{2.5}$ গড় ৫.৪ $\mu g/m^3$)**। এটি হলো কোল্ড-এয়ার ব্রঙ্কোস্পাজম (Cold-Induced Asthma)।
   - **ফলাফল:** মডেল রোগী ৩৪৩-এর পরিষ্কার বাতাসে শীতকালীন অ্যাটাককে রেড বলতে পারেনি, তবে কস্ট-সেন্সিটিভ ওজনের কারণে সতর্কতামূলকভাবে At-Risk (Yellow) গেটে সফলভাবে আটকে দিয়েছে (Red $\to$ Green FN = 0)।
2. **বাধ্যতামূলক ক্লিনিক্যাল সীমাবদ্ধতা (Mandatory Clinical Limitations):**
   - **রিসার্চ প্রক্সি লেবেল:** পিক ফ্লো রেশিও ভিত্তিক লেবেলগুলো ক্লিনিক্যাল ডায়াগনোসিস নয়, গবেষণার জন্য প্রস্তুতকৃত প্রক্সি লেবেল।
   - **রেড স্যাম্পল সীমাবদ্ধতা:** সম্পূর্ণ ডেটাসেটে রেড স্যাম্পলের সংখ্যা অত্যন্ত কম (টেস্টে মাত্র ৪টি এবং ক্রোনোলজিক্যালে ২টি), যার ফলে আর্কিটেকচারের পারস্পরিক শ্রেষ্ঠত্ব পরিসংখ্যানগতভাবে অপ্রমাণিত।
   - **ক্লিনিক্যাল কার্যকারিতা অপ্রমাণিত:** বাহ্যিক ক্লিনিক্যাল ট্রায়াল ছাড়া এই মডেলটিকে ক্লিনিক্যালি ভ্যালিডেটেড বা চিকিৎসায় স্বয়ংক্রিয় সিদ্ধান্তের জন্য নিরাপদ দাবি করা নিষিদ্ধ।

---

### 5.7 Cryptographic Reproducibility Manifest & Release Tag

গবেষণার প্রতিটি ধাপের ডাটা, স্ক্রিপ্ট ও মডেল ক্রিপ্টোগ্রাফিক হ্যাশ দ্বারা ফ্রোজেন ও সংরক্ষিত:
- **অ্যানোটেটেড গিট ট্যাগ:** `v1.0.0-audit-baseline` (কমিট `36a29e36f727b5fc3b08eeccf5ef281b97aea965`)
- **রিলিজ আর্কাইভ বান্ডেল:** `scratch/audit_baseline_v1_frozen.tar.gz` (সাইজ: ৯৭৭,১৫৭ বাইট, SHA-256: `365f0b6ca7579d4ccd57c2516327eced0ac073971f8b1eb1c26168e073033c76`)
- **নির্বাচিত অডিট-সংবেদনশীল ফাইলসমূহের SHA-256 হ্যাশ:**

| ফাইল পাথ | ফাইলের আকার | SHA-256 চেকসাম | ফাইলের ভূমিকা ও অখণ্ডতা |
| :--- | :---: | :--- | :--- |
| `datasets/raw/anonym_aamos00_patient_info.csv` | 4,130 B | `0563743582c295a87d488c8f51c7f7682fa51b25cadb91bc83b7af8201872644` | মূল ক্লিনিক্যাল ইনটেক মেটাডাটা (২২ রোগী) |
| `datasets/raw/anonym_aamos00_peakflow.csv` | 53,386 B | `0b211e61d4aaa4613d25e95777af03ff535767b744ef79f38ef2722d2374ba83` | মূল স্পাইরোমেট্রি পিক ফ্লো ডাটা |
| `datasets/raw/anonym_aamos00_environment.csv` | 197,632 B | `943b5a6e3519f6c16893389f3017cf2d1b270cf4526076dee2cf9d6361400158` | মূল পরিবেশগত সেন্সর টাইম সিরিজ |
| `datasets/generated/asthma_risk_extended.csv` | 177,093 B | `c9b672799b201e6e0e93691e476f7c9a9764324847245ee076849e5fc35d4866` | আদি প্রোডাকশন ডাটা (ঐতিহাসিক লিকযুক্ত) |
...
| `scratch/patient_split_predictions.json` | 19,439 B | `0f59e8f529e413a107e5df3c2b40b1d0d7aee44baf61bdb05b860265fe57de5e` | ফ্রোজেন পেশেন্ট স্প্লিট প্রেডিকশন ভেক্টর |
| `scratch/chronological_split_predictions.json` | 22,954 B | `10b4ae4c8fb721e2c6e430f606b94c0ea17d9f5193da0fafee39e2db9adab71e` | ফ্রোজেন ক্রোনোলজিক্যাল প্রেডিকশন ভেক্টর |
